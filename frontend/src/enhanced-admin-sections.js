// Улучшенные разделы админ-панели с загрузкой изображений и управлением категориями
...
// Раздел событий с загрузкой изображений
        ...
        <div key={event.id} className="bg-white rounded-lg p-6 shadow">
          <div className="flex justify-between items-start mb-4">
            <div className="flex-1">
              ...
              {event.image && (
                <img src={event.image} alt={event.title_ru} className="w-full h-32 object-cover rounded-lg mb-3" />
              )}
...
// Раздел аккаунтов (без изменений)
...
// Раздел руководства с загрузкой изображений
...
// Улучшенный раздел настроек с соц. сетями
export const EnhancedSettingsSection = ({ adminData, updateSiteSettings, updateSeoSettings }) => {
  ...
  const [seoSettings, setSeoSettings] = React.useState(adminData.seoSettings || {
    ...
    keywords: 'нейрохирургия, мозг, операции, Узбекистан'
  });
  ...
  const removePhone = (index) => {
    const newPhones = siteSettings.phones.filter((_, i) => i !== index);
  ...
  const removeEmail = (index) => {
    const newEmails = siteSettings.emails.filter((_, i) => i !== index);
        ...
        <div className="bg-white rounded-lg p-6 shadow">
          ...
          <form onSubmit={handleSiteSettingsSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                type="url"
                value={siteSettings.socialMedia?.website || ''}
                onChange={(e) => setSiteSettings({
                  ...siteSettings,
                  socialMedia: {...siteSettings.socialMedia, website: e.target.value}
                })}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500"
...
// Раздел управления категориями галереи
export const GalleryCategoriesSection = ({ 
  categories, 
  setCategories, 
  ...
  const deleteCategory = (id) => {
    if (confirm('Удалить эту категорию? Все изображения из неё будут перенесены в "Общее".')) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {categories.map((category) => (
          <div key={category.id} className="border border-gray-200 rounded-lg p-4">
            <h4 className="font-medium text-gray-900 mb-2">{category.name_ru}</h4>
            <p className="text-sm text-gray-600 mb-3">{category.description_ru}</p>
            
            <div className="text-xs text-gray-500 mb-3">
              <span className="inline-block px-2 py-1 bg-gray-100 rounded mr-2">RU: ✓</span>
              <span className="inline-block px-2 py-1 bg-gray-100 rounded mr-2">UZ: {category.name_uz ? '✓' : '✗'}</span>
              <span className="inline-block px-2 py-1 bg-gray-100 rounded">EN: {category.name_en ? '✓' : '✗'}</span>
            </div>
